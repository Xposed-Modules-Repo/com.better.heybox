...
- **自动完成每日分享任务**：自动完成小黑盒每日任务
- **3 个独立链接设置**：图片帖链接 / 普通帖链接 / 游戏链接，各自独立配置
  （支持 `api.xiaoheihe.cn/...web/share?link_id=`、`xiaoheihe.cn`、`heybox://` 链接，
  经小黑盒 RouterActivity 自动路由；未配置的类型自动跳过）；每日状态按日期记录，跨天重置
